...
requestSigner.newClient = function(config) {
  ...
  let awsRequestSigner = {};
  if (config.accessKey === undefined || config.secretKey === undefined) {
    return awsRequestSigner;
  }
  awsRequestSigner.accessKey = config.accessKey;
  awsRequestSigner.secretKey = config.secretKey;
  awsRequestSigner.sessionToken = config.sessionToken;
  awsRequestSigner.serviceName = config.serviceName || 'execute-api';
  awsRequestSigner.region = config.region || 'us-east-1';
  awsRequestSigner.defaultAcceptType =
    config.defaultAcceptType || 'application/json';
  awsRequestSigner.defaultContentType =
  ...
  awsRequestSigner.signRequest = function(request) {
    ...
    // If the user has not specified an override for Content type the use
    // default
    if (headers['Content-Type'] === undefined) {
      headers['Content-Type'] = awsRequestSigner.defaultContentType;
    }

    // If the user has not specified an override for Accept type the use default
    if (headers['Accept'] === undefined) {
      headers['Accept'] = awsRequestSigner.defaultAcceptType;
    }
    ...
    let parser = new URL(awsRequestSigner.endpoint);
    ...
    if (
      ...
      awsRequestSigner.sessionToken !== ''
    ) {
      headers[X_AMZ_SECURITY_TOKEN] = awsRequestSigner.sessionToken;
    }
    delete headers[HOST];

    let url = awsRequestSigner.endpoint + path;
